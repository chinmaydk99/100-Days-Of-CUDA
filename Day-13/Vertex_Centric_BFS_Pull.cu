...
__global__ void bfs_pull(CSCGraph cscGraph, unsigned int *level, unsigned int *newVertexVisited, unsigned int currLevel){
    ...
    if(vertex < numVertices){
        if(level[vertex] == UINT_MAX){// If current node is univisited
            for(unsigned edge = cscGraph.dstPtrs[vertex]; edge < cscGraph[vertex+1]; edge ++){
...
int main() {
    ...
    CSCGraph d_cscGraph;
    cudaMalloc(&d_cscGraph.dstPtrs, (numVertices + 1) * sizeof(int));
    cudaMalloc(&d_cscGraph.src, numEdges * sizeof(int));
    cudaMemcpy(d_cscGraph.dstPtrs, h_dstPtrs, (numVertices + 1) * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_cscGraph.src, h_src, numEdges * sizeof(int), cudaMemcpyHostToDevice);
